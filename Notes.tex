...
\section{Vector Spaces and Linear Transformations}
 ...
 \qs{}{
 Let \(T: V \to W\) be a linear transformation from a vector space \(V\) to a vector space \(W\). Show that \(\img(T)\) is a subspace of \(W\).
 }
 
 \sol{
 \(0 \in \img(T)\), since \(T\) is linear, hence \(T(0) = 0\).
 
 If \(w_1, w_2 \in \img(T)\), then there exists \(v_1, v_2\) such that \(T(v_1) = w_1, \ T(v_2) = w_2\). Then,
 
 \[
 w_1 + w_2 = T(v_1) + T(v_2) = T(v_1 + v_2) \in \img(T)
 \]
 
 Also, if \(\lambda\) is a scalar,
 
 \[
 \lambda w_1 = \lambda T(v_1) = T(\lambda v_1) \in \img(T)
 \]
 
 Due to these criteria \(\img(T)\) is a subspace of \(W\).
 
 \Qed
 }
 
 \qs{}{
 Let \(A, B\) be two invertible matrices. Show that
 ...
 Since \((AB)(B^{-1}A^{-1}) = (B^{-1}A^{-1})(AB) = I\), it is clear that \((AB)^-1 = B^{-1}A^{-1}\).
 ...
 \sol{
 We first need to find the eigenvalues for the matrix.
 
 \[
 \det(A-\lambda I) = (2-\lambda)(5-\lambda) + 2 = \lambda^2 -7\lambda + 12 = (\lambda - 3)(\lambda - 4)
 \]
 ...
 Since \(Av = \lambda v\), \(Av - \lambda v = 0\), \((Av - \lambda I)v = 0\)
 ...
 \begin{array}{cc|c}
 	...
 	0 & 0  & 0 \\
 \end{array}
 \quad \implies \quad v_1 = \colvec{-2\\1}
 \]
 ...
 \begin{array}{cc|c}
 	...
 	0 & 0  & 0 \\
 \end{array}
 \quad \implies \quad v_2 = \colvec{-1\\1}
 \]
